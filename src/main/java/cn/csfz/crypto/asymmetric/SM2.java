...
public class SM2 extends AbstractAsymmetricCrypto<SM2> {
     ...
     * * 在硬件签名中，返回结果为R+S，可以通过调用{@link cn.csfz.crypto.SmUtil#rsAsn1ToPlain(byte[])}方法转换之。
     *
     * @param data 加密数据
     * @return 签名
     */
    ...
    private CipherParameters getCipherParameters(KeyType keyType) {
        switch (keyType) {
            ...
            case PrivateKey:
                if (this.privateKeyParams == null) {
                    throw new IllegalArgumentException("PrivateKey must be not null !");
                }
    ...
    private SM2Engine getEngine() {
        if (null == this.engine) {
            if (this.digest == null) {
                throw new RuntimeException("digest must be not null !");
            }
